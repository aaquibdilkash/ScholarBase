...
export async function createComment(
  ...
  if (mentionsRaw) {
    try {
      ...
      const validMentions = mentions?.filter((m): m is { id: string; handle: string } => Boolean(m.handle)) ?? []
...
export async function deleteComment(commentId: string, type: CommentEntityType) {
  const { id: userId } = await requireCurrentUser('You must be logged in to delete comments.')
  ...
  return { success: true, data: { id: commentId, parentId: comment?.parentId, wasTombstoned: result.wasTombstoned } }
}
