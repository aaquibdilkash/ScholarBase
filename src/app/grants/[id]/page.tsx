...
import { getResearchGrantById, deleteResearchGrant } from "@/app/actions/grants";
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    const description = grant.amount ? `${grant.title} funding opportunity: ${grant.amount}.` : `Research grant opportunity: ${grant.title}.`;
    return {
      title: grant.title,
      description,
      alternates: { canonical: `/grants/${grant.id}` },
      openGraph: { title: grant.title, description, type: "article", url: `/grants/${grant.id}` },
    };
  } catch {
...
export default async function ResearchGrantDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
    ...
    >
      ...
      {grant.amount && (
        <p className="mb-4 text-sm text-slate-600 dark:text-slate-300 sm:text-base">
          <span className="font-semibold text-slate-800 dark:text-slate-100">Amount:</span> {grant.amount}
        </p>
      )}
      <RichContent content={grant.description} />
      <div className="mt-4 flex flex-col gap-3 sm:flex-row">
        {grant.applyLink && <a href={grant.applyLink} target="_blank" rel="noopener noreferrer" className="flex-1 rounded-lg bg-slate-950 py-2 text-center text-xs font-semibold text-white transition-colors hover:bg-slate-800 sm:text-sm">Apply</a>}
        {grant.infoLink && <a href={grant.infoLink} target="_blank" rel="noopener noreferrer" className="flex-1 rounded-lg border border-slate-200 py-2 text-center text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-800 dark:text-slate-200 dark:hover:bg-slate-900 sm:text-sm">More Info</a>}
      </div>
    </DetailPageCardShell>
  );
}
