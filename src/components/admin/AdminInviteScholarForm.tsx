    ...
    <div className="space-y-6">
      ...
      <div className="grid items-stretch gap-6 lg:grid-cols-2">
        ...
        <section className={`${activeTab === "compose" ? "hidden lg:flex" : "flex"} h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900`}>
          ...
          <div className="border-b border-slate-200 bg-slate-50 px-5 py-3 text-xs dark:border-slate-800 dark:bg-slate-950/70 sm:px-6"><div className="space-y-1"><div><span className="text-slate-400">From:</span> <span className="font-medium text-slate-700 dark:text-slate-200">ScholarBase &lt;invitations@scholarbase.app&gt;</span></div><div><span className="text-slate-400">To:</span> <span className="font-medium text-slate-700 dark:text-slate-200">{draft.scholarEmail || "recipient@university.edu"}</span></div><div><span className="text-slate-400">Subject:</span> <span className="font-medium text-slate-700 dark:text-slate-200">{draft.subject || "Only For Scholars: A Transparent Introduction to ScholarBase"}</span></div></div></div>
