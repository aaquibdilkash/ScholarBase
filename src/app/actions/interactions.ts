...
const VOTE_CONFIG: Record<string, {
  ...
  researchGrant: {
    voteModel: prisma.researchGrantVote,
    uniqueFields: ['researchGrantId', 'userId'],
    targetIdField: 'researchGrantId',
    notifType: 'research-grant-upvoted',
    urlPrefix: '/grants/',
    titleField: 'title',
  },
  course: {
    voteModel: prisma.courseVote,
    uniqueFields: ['courseId', 'userId'],
    targetIdField: 'courseId',
    notifType: 'course-upvoted',
    urlPrefix: '/learn/',
    titleField: 'title',
  },
  result: {
...
const CONTENT_AUTHOR_FETCH: Record<string, (targetId: string) => Promise<{ authorId: string; title: string } | null>> = {
  ...
  researchGrant: (id) => prisma.researchGrant.findUnique({ where: { id }, select: { authorId: true, title: true } }),
  course: (id) => prisma.course.findUnique({ where: { id }, select: { authorId: true, title: true } }),
...
const COMMENT_REPUTATION_CONFIG: Record<string, { commentTable: string; voteTable: string; contentField: string }> = {
  ...
  researchGrant: { commentTable: 'ResearchGrantComment', voteTable: 'ResearchGrantCommentVote', contentField: 'researchGrantId' },
  course: { commentTable: 'CourseComment', voteTable: 'CourseCommentVote', contentField: 'courseId' },
