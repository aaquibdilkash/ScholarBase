...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    const bits = [course.provider, course.instructor].filter(Boolean).join(" by ");
    const description = bits ? `${course.title}: a research learning course from ${bits}.` : `Research learning course: ${course.title}.`;
    return {
      title: course.title,
      description,
      alternates: { canonical: `/learn/${course.id}` },
      openGraph: { title: course.title, description, type: "article", url: `/learn/${course.id}` },
    };
  } catch {
    return { title: "Course" };
  }
...
export default async function CourseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    ...
    >
      ...
      <a href={course.link} target="_blank" rel="noopener noreferrer" className="mt-4 block rounded-lg bg-slate-950 py-2 text-center text-xs font-semibold text-white transition-colors hover:bg-slate-800 sm:text-sm">
        Open Course
      </a>
    </DetailPageCardShell>
  );
