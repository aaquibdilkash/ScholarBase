...
import {
  ...
  JournalReviewRatingDistributionRow,
} from "./journalReviewCount";

/**
 * The Overall Rating card rendered on the journal detail page.
 * Mirrors `OverallRatingSection` for supervisors: seeded from server
 * values, then re-derives the average, count, and star bars from the
 * shared aggregates store — every delete updates it instantly with NO
 * fetch / router.refresh.
 */
export function JournalRatingSection({
  journalId,
  initialCount,
  initialAvgRating,
  initialDistribution,
  ...
  initialDistribution: JournalReviewRatingDistributionRow[];
}) {
  ...
  const { data: agg } = useQuery<JournalReviewAggregates>({
  ...
  return (
    <div className="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-slate-200/60 p-3 sm:p-4 md:p-5 lg:p-6 mt-4 sm:mt-6 mb-6 sm:mb-8">
