...
export async function createRecommendation(
  ...
  if (existing) {
    return {
      success: false,
      error: "You already have a recommendation for this supervisor.",
    };
  }
  ...
  try {
    ...
    if (
      ...
      return {
        success: false,
        error:
          "You have already submitted a recommendation for this supervisor.",
      };
    }
    throw error;
  }
...
export async function updateRecommendation(
  ...
  try {
    ...
    const updatedRecommendation = await prisma.$transaction(async (tx) => {
      ...
      const countDelta =
        recommendation.isAnonymous === isAnonymous ? 0 : isAnonymous ? -1 : 1;
      await tx.supervisor.update({
        ...
        data: {
          ...(ratingDelta !== 0
            ...
            : {}),
          ...(countDelta !== 0
            ? { recommendationCount: { increment: countDelta } }
            : {}),
        },
      });
